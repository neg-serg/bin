...
function display() {
    ...
    emoji=$(printf "$emoji\n$update")
    line=$(echo "$emoji" | rofi -sort -match fuzzy -case-sensitive=false -dmenu -i -p emoji -kb-custom-1 Ctrl+1 $@)
    exit_code=$?

    if [ "${line[@]}" == "$update" ]; then
        download
        if which notify-send 2>&1 1>/dev/null; then
            notify-send "$(basename $0)" "Emoji cache successfully updated"
        fi
        exit 0
    fi
